...
fn entrypoint(inputs: Vec<ValuePresenter>) -> Outputs {
    ...
    let vertices_id = match extract_numeric_field_value(
        inputs
            .get(2)
            .expect("flow id is not passed with valid parameters"),
    ) {
        Ok(value) => {
            if let Some(number_value) = value {
                ...
                panic!("The vertices id passed in does not have a default value, please do not pass in nil or none")
            }
        }
        Err(err) => {
            panic!("Error: {}", err.message);
    ...
    let authorization_token = match extract_single_line_field_value(inputs.get(3).expect("base url is not passed with valid parameters")) {
        Ok(value) => {
    ...
    let url = format!("{base_url}/api/v4/yaw/flows/{flow_id}/vertices/{vertices_id}");
