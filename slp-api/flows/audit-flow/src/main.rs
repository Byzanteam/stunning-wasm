...
fn entrypoint(inputs: Vec<ValuePresenter>) -> Outputs {
    ...
    let journey_id = extract_value(other_value.get(0).unwrap(), "flow_id")
        .unwrap_or_else(|err| panic!("{}", err.message));
    let assignment_id = extract_value(other_value.get(1).unwrap(), "flow_id")
        .unwrap_or_else(|err| panic!("{}", err.message));
    let body = extract_value(other_value.get(1).unwrap(), "body")
        .unwrap_or_else(|err| panic!("{}", err.message));
    let url = format!("{base_url}/api/v4/yaw/journeys/{journey_id}/assignments/{assignment_id}");
    let request_data = NetworkingRequest::put(url, headers, Some(body));
