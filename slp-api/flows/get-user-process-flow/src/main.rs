...
fn entrypoint(inputs: Vec<ValuePresenter>) -> Outputs {
    ...
    let user_id = extract_value(other_value.get(0).unwrap(), "flow_id")
        .unwrap_or_else(|err| panic!("{}", err.message));
    let category = extract_value(other_value.get(1).unwrap(), "flow_id")
        .unwrap_or_else(|err| panic!("{}", err.message));
    let url = format!(
        "{base_url}/api/v4/yaw/flows/user_assignments.json?user_id={user_id}&category={category}"
    );
